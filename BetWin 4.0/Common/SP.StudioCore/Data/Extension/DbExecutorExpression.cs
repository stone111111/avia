...
namespace SP.StudioCore.Data.Extension
{
    ...
    public static class DbExecutorExpression
    {
        ...
        /// <param name="fields"></param>
        /// <returns></returns>
        public static IDataReader ReadData<T>(this DbExecutor db, params Expression<Func<T, object>>[] fields) where T : class, new()
        {
            string field = "*";
            if (fields.Length != 0)
            {
        ...
        /// <param name="condition"></param>
        /// <returns></returns>
        public static List<TValue> ReadData<T, TValue>(this DbExecutor db, Expression<Func<T, TValue>> field, Expression<Func<T, bool>> condition) where T : class, new()
        {
        ...
        /// <param name="fields"></param>
        /// <returns></returns>
        public static List<T> ReadList<T>(this DbExecutor db, params Expression<Func<T, object>>[] fields) where T : class, new()
        {
            List<T> list = new List<T>();
            using (IDataReader reader = db.ReadData(fields))
            {
                try
                {
                    while (reader.Read())
