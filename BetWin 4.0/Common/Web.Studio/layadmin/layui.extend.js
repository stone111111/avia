...
!function (ns) {
    ...
    ns["jsonViewer"] = function (elemId, json) {
        ...
        layui.link("https://img-cdn.a8.to/studio/js/jquery.json-viewer.css");
        if (elem.jsonViewer) {
            elem.jsonViewer(json);
        } else {
            jQuery.getScript("https://img-cdn.a8.to/studio/js/jquery.json-viewer.js", function () {
                elem.jsonViewer(json);
            });
        }
    };
...
!function (ns) {
    ...
    ns["number"] = function (value, dec) {
        ...
        return negative + arr.join(",") + (dec ? decimal.toFixed(dec).substr(1) : "");
    };
...
!function () {
    ...
    Element.prototype.getData = function () {
        ...
        for (var i = 0; i < obj.attributes.length; i++) {
            ...
            var name = item.name;
            if (name.indexOf("data-") === -1) continue;
    ...
    // 获取表单元素的值
    Element.prototype.getField = function () {
        var elem = this;
        var data = {};
