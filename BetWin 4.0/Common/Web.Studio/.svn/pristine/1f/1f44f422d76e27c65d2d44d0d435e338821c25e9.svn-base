...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function (exports) {
    ...
    var $ = layui.$
        ...
        , ELEM_VIEW = 'layui-table-view', ELEM_HEADER = '.layui-table-header', ELEM_BODY = '.layui-table-body', ELEM_FOOT = '.layui-table-tfoot', ELEM_MAIN = '.layui-table-main', ELEM_FIXED = '.layui-table-fixed', ELEM_FIXL = '.layui-table-fixed-l', ELEM_FIXR = '.layui-table-fixed-r', ELEM_TOOL = '.layui-table-tool', ELEM_PAGE = '.layui-table-page', ELEM_SORT = '.layui-table-sort', ELEM_EDIT = 'layui-table-edit', ELEM_HOVER = 'layui-table-hover'
        ...
        //tfoot区域模板 老司机309
        , TPL_FOOTER = function (options) {
            options = options || {};
            return ["{{# if(d.data.footer){ }}",
                "<tfoot class='layui-table-tfoot'></tfoot>",
                "{{# } }}"].join("");
        }
    ...
    Class.prototype.render = function () {
        ...
        that.layFoot = reElem.find(ELEM_FOOT);
        ...
        if (options.url) { //Ajax请求
            ...
            $.ajax({
                ...
                , success: function (res) {
                    // 老司机309 自定义的格式转换
                    if (response.covert) res = response.covert(res);
    ...
    Class.prototype.renderData = function (res, curr, count, sort) {
        var that = this
            ...
            // 数据统计字段 老司机309
            , totalData = res[options.response.totalName] || null
            , trs = []
            , trs_fixed = []
            , trs_fixed_r = []
            , tfoot = []
            ...
            , render = function () { //后续性能提升的重点
                ...
                layui.each(data, function (i1, item1) {
                    ...
                    that.eachCols(function (i3, item3) {
                        ...
                        var td = ['<td data-field="' + field + '" ' + function () {
                            var attr = [];
                            if (item3.dragsort) attr.push('data-dragsort="' + item3.dragsort + '"');    // 是否允许拖动排序 老司机309
                        ...
                        function () {
                            ...
                            // 手选拖动框架
                            if (item3.dragsort) return "<drag draggable=\"true\"></drag>";
                ...
                // 底部统计条渲染
                if (that.config.footer && totalData && trs.length > 0) {
                    tfoot.push("<tr>");
                    layui.each(that.config.footer, function (index, item) {
                        tfoot.push("<td>");
                        if (item) {
                            if (item.title) {
                                tfoot.push(item.title);
                            } else if (item.field) {
                                tfoot.push(totalData[item.field]);
                            } else if (item.templet) {
                                tfoot.push(function () {
                                    return typeof item.templet === 'function'
                                        ? item.templet(tplData)
                                        : laytpl(item.templet).render(totalData)
                                }());
                            }
                        }
                        tfoot.push("</td>");
                    ...
                    tfoot.push("</tr>");
                    that.layMain.find('tfoot').html(tfoot.join(''));
                }
    ...
    // 拖动排序事件   老司机309
    Class.prototype.dragSort = function () {
        var that = this,
            options = that.config;

        var data = table.cache[that.key]
        var target_trs = that.layMain.find("tr");
        var drags = that.layMain.find("drag");
        drags.on("dragstart", function (e) {
            var tr = $(this).parents("tr");
            e.originalEvent.dataTransfer.setData('tableDragIndex', tr.attr("data-index"));
        });
        target_trs.on("dragenter", function (e) {
            $(this).addClass("drag-over");
            e.preventDefault();
        });
        target_trs.on("dragover", function (e) {
            $(this).addClass("drag-over");
            e.originalEvent.dataTransfer.dropEffect = "link"
            e.preventDefault();
        });
        target_trs.on("dragleave", function (e) {
            $(this).removeClass("drag-over");
            e.preventDefault();
        });
        target_trs.on("drop", function (e) {
            var target = $(this);
            var targetIndex = target.attr("data-index");    // 目标
            var source = Number(e.originalEvent.dataTransfer.getData('tableDragIndex'));    // 来源
            if (targetIndex == source) return;

            if (targetIndex > source) {
                data[source][table.config.indexName] = targetIndex - 1;
                for (var i = source + 1; i < targetIndex; i++) {
                    data[i][table.config.indexName]--;
                }
            } else {
                data[source][table.config.indexName] = targetIndex;
                for (var i = targetIndex; i < source; i++) {
                    data[i][table.config.indexName]++;
                }

            }
            var thisData = data.sort(function (item1, item2) {
                var index1 = Number(item1[table.config.indexName]);
                var index2 = Number(item2[table.config.indexName]);
            ...
            console.log(options.dragsort);
            if (options.dragsort && typeof options.dragsort == "function") {
                options.dragsort(data);
            }
        });
    };
