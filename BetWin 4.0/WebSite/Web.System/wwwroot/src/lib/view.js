...
layui.define(['laytpl', 'layer'], function (exports) {
    ...
    view.req = function (options) {
        ...
        if (options.url) {
            if (layui.setter.apiurl && !/^http|^\/\//.test(options.url)) {
                if (layui.setter.apipath && !layui.setter.debug) {
                    options.headers[layui.setter.apipath] = window.btoa ? window.btoa(options.url) : options.url;
                    options.url = layui.setter.apiurl;
                } else {
    ...
    Class.prototype.parse = function (html, refresh, callback) {
        ...
        for (var i = elemTemp.length; i > 0; i--) {
            (function () {
                ...
                if (url) {
                    switch (dataElem.attr('lay-type')) {
                        case "control":
                            var dataElemId = dataElem.attr("id");
                            if (!dataElemId) {
                                dataElemId = "control-" + new Date().getTime();
                                dataElem.attr("id", dataElemId);
                            }
                            view(dataElemId).render(url, data);
                            break;
                        default:
                            view.req({
