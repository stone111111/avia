    ...
    betwin.form = {
        submit: function (filter, data, callback) {
            ...
            form.on("submit(" + filter + ")", function (e) {
                var formObj = e.form;
                var postData = data(e.field, formObj);
            ...
            !function () {
                layui.each(formObj.find("[data-date]"), function (index, item) {
                    ...
                    switch (item.getAttribute("data-date-value")) {
                        case "Today":
                            item.value = new Date().formatDate("yyyy-MM-dd"); p
                            break;
                    }
                });
            }();
    ...
    betwin.admin = {
        popup: function (id) {
            ...
            page.on("click", "[data-popup]", function (e) {
                ...
                var data = obj.getAttribute("data-data") === null ? {} : JSON.parse(obj.getAttribute("data-data"));
                ...
                var req = function () {
                    betwin.admin.req({
                        ...
                        complete: function (res) {
                            if (callback && BW.callback[callback]) BW.callback[callback](res);
                        }
                    });
                };
                if (obj.getAttribute("data-confirm")) {
                    layer.confirm(obj.getAttribute("data-confirm"), {
                        title: "確認操作",
                        icon: 3
                    }, function (index) {
                        req();
        ...
        req: function (options) {
            ...
            admin.req({
                ...
                success: function (res) {
                    ...
                    if (res.success && options.success) options.success(res);
                }
            });
        }
    };
